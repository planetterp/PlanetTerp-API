...
class Grades:
    def GET(self):
        model.insert_view(
            ...
            web.ctx.status,
            web.ctx.ip,
            web.ctx.env['HTTP_USER_AGENT'] if 'HTTP_USER_AGENT' in web.ctx.env else None,
            "GET")
        ...
        if 'professor' not in data and 'course' not in data:
            return utilities.api_error(
                "parameters must include at least one of: \"course\", \"professor\"")
        ...
        if 'semester' in data:
            ...
            if not data['semester'] in possible_semesters:
                return utilities.api_error(
                    "invalid semester parameter; semester parameter must be one of the following: " +
                    ', '.join(possible_semesters))
        ...
        if 'section' in data:
            ...
            if not any(section['section'] == data['section']
                       for section in all_sections):
                return utilities.api_error(
                    "invalid section; example of a valid section: 0101")
        ...
        for course_grade in grades:
            if course_grade['department'] and course_grade['course_number']:
                course = course_grade['department'] + \
                    course_grade['course_number']
            else:
                course = None
            grades_data.append({
                'course': course,
